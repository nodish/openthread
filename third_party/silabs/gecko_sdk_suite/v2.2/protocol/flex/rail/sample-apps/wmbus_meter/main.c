...
int main(void)
{
  ...
  while (1) {
    switch (state) {
      ...
      break;
      case S_RESTART_TIMER:
      {
        //convert lastEventTime into RAIL timebase
        CORE_DECLARE_IRQ_STATE;
        CORE_ENTER_CRITICAL();
        uint32_t currentTime = RTCDRV_GetWallClockTicks32();
        gLastEventTime = RAIL_GetTime() - 1000 * RTCDRV_TicksToMsec((uint32_t)(currentTime - gLastEventTime));
        CORE_EXIT_CRITICAL();
        ...
        CORE_ENTER_CRITICAL();
        gWmbusAppPeriodAcc -= (RAIL_GetTime() - gLastEventTime) / 1000;
        RTCDRV_StartTimer(gWmbusPeriodicTimer, rtcdrvTimerTypeOneshot, gWmbusAppPeriodAcc, wmbusPeriodicTimerCb, NULL);
        CORE_EXIT_CRITICAL();
      }
        state = S_STARTTX;
...
void wmbusPeriodicTimerCb(RTCDRV_TimerID_t id, void *user)
{
  //if we're waking up from sleep, we don't yet have RAILTime running, so we need to convert it later
  gLastEventTime = RTCDRV_GetWallClockTicks32();
  state = S_RESTART_TIMER;
}
