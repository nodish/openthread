...
{
  uint32_t pageSize = SYSTEM_GetFlashPageSize();
...
nvm3_HalWriteSize_t nvm3_halGetWriteSize(void)
{
  ...
  return NVM3_HAL_WRITE_SIZE_32;
#else
  return NVM3_HAL_WRITE_SIZE_16;
#endif
}

uint8_t nvm3_halGetDeviceId(void)
{
  return _SILICON_LABS_GECKO_INTERNAL_SDID;
}
...
#if CHECK_DATA
  if (halSta == ECODE_NVM3_OK) {
    if (!isErased(nvmAdr, nvm3_halGetPageSize())) {
